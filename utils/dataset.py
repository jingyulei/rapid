...
from utils.dataset_utils import normalize_pose, normalize_pose_bbox, normalize_pose_robust, gen_clip_seg_data_np, ae_trans_list, normalize_pose_stan
import random
...
class PoseDataset(Dataset):
    def __init__(self, path_to_json_dir,
        ...
        self.vid_res = dataset_args.get('vid_res', [1920,1080])
    ...
    def gen_dataset(self, ret_keys=False, **dataset_args):
        ...
        if self.headless:
            non_head_indices = list(range(1, 14)) + list(range(18, 75))
            segs_data_np = segs_data_np[:, :, non_head_indices]
...
class PoseDatasetRobust(PoseDataset):
    ...
    def gen_dataset(self, ret_keys=False, **dataset_args):
        ...
        X_local = X_local.reshape((*X_local.shape[:2], 75, 2))
...
def get_dataset_and_loader(args, split='train'):
