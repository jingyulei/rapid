...
from utils.eval_utils import (compute_var_matrix, filter_vectors_by_cond, pad_scores, score_process)
...
class RAPID(pl.LightningModule):
    ...
    def __init__(self, args:argparse.Namespace) -> None:
        """
        This class implements RAPID model.
    ...
    def forward(self, input_data:List[torch.Tensor], aggr_strategy:str=None, return_:str=None) -> List[torch.Tensor]:
        ...
        #######################################################################################################
        # visualize_frame_and_diffusion(corrupt_data, generated_xs) #for visualize latent sphere 
        ########################################################################################################
    ...
    def on_test_epoch_end(self) -> float:
        ...
        auc = self.post_processing(out, gt_data, trans, meta, frames)
    ...
    def post_processing(self, out:np.ndarray, gt_data:np.ndarray, trans:np.ndarray, meta:np.ndarray, frames:np.ndarray) -> float:
        ...
        # computing ROC Curve
        fpr, tpr, _ = roc_curve(gt, pds)
        
        plot_roc_curve(fpr, tpr)
        plot_predictions_vs_actuals(pds, gt)
    ...
    def test_on_saved_tensors(self, split_name:str) -> float:
        ...
        auc = self.post_processing(tensors['prediction'], tensors['gt_data'], tensors['trans'],
    ...
    def _select_frames(self, data:torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, List[torch.Tensor]]:
        ...
        if self.conditioning_strategy == 'random_imp':
            ...
            cond_idxs = torch.tensor(self.conditioning_indices, device=self.device)#cond_idxs=[0,1,2]
...
def plot_roc_curve(fpr, tpr):
    ...
    This function takes the false positive rate (fpr) and true positive rate (tpr) 
    as inputs, calculates the AUC (Area Under the Curve), and generates a plot 
    displaying the ROC curve along with the AUC value. The plot is saved as an image 
    in the './pictures' directory.

    Args:
        fpr (torch.Tensor or array-like): The false positive rate values.
        tpr (torch.Tensor or array-like): The true positive rate values.

    Returns:
        None: The function does not return any value. It only generates and saves the plot.

    Saves:
        A plot of the ROC curve is saved as 'roc_curve.png' in the './pictures' directory.
    ...
    plt.plot(fpr, tpr, color=sns.color_palette("coolwarm")[1], lw=3, label=f'ROC Curve (AUC = {auc(fpr, tpr):.2f})')
    ...
    plt.title('Receiver Operating Characteristic (ROC) Curve', fontsize=40)  
...
def plot_predictions_vs_actuals(pds, gt):
    """
    Plots the predicted scores against the actual labels for a series of frames.

    This function takes predicted scores (`pds`) and ground truth labels (`gt`), 
    and generates a plot that visualizes how well the predicted scores align with 
    the actual labels. The plot is saved as an image in the './pictures' directory.

    Args:
        pds (numpy.ndarray or list): A list or array of predicted scores, 
                                      where each score represents the predicted confidence for a frame.
        gt (numpy.ndarray or list): A list or array of ground truth labels (0 or 1),
                                     where 1 represents the actual label for a positive frame, and 0 for a negative frame.

    Returns:
        None: The function does not return any value. It only generates and saves the plot.

    Saves:
        A plot comparing predicted scores with actual labels, saved as 'pred_vs_actual.png' 
        in the './pictures' directory.
    ...
    sample_indices = np.arange(len(pds))
    
    plt.plot(sample_indices, np.minimum(pds * 20, 1), label='Predicted Scores', color='#F28B82', lw=1.5)
    plt.fill_between(sample_indices, 0, 1, where=(gt == 1), color=sns.color_palette("coolwarm")[0], alpha=0.3, step='pre', lw=0, label='Actual Labels')
    plt.xlabel('Frame Index', fontsize=30)  
    plt.ylabel('Predicted Scores', fontsize=30)  
    plt.title('Predicted Scores vs Actual Labels', fontsize=30)  
    ...
    plt.savefig(os.path.join(output_dir, 'pred_vs_actual.png'), dpi=300, bbox_inches='tight')
    plt.show()



    
def visualize_frame_and_diffusion(corrupt_data, generated_xs):
    
    """
    Visualizes a frame and its corresponding diffusion results.

    This function visualizes an actual frame from the corrupted data and compares it with 
    the generated diffusion results from different time steps using t-SNE for dimensionality 
    reduction. The visualization is displayed in a 3D scatter plot and saved as an image.

    Args:
        corrupt_data (torch.Tensor): The actual input tensor data with shape (B, C, T, V), 
                                      where B is the batch size, C is the number of channels, 
                                      T is the number of time steps, and V is the number of frames.
        generated_xs (list of torch.Tensor): A list of tensors representing the diffusion results 
                                              generated at different time steps. Each tensor has 
                                              shape (B, C, T, V).
    ...
    Saves:
        A 3D scatter plot visualizing the actual frame and diffusion results, saved as 
        'visualization_frame.png' in the './pictures' directory.

    """
    
    output_dir = './pictures'
    os.makedirs(output_dir, exist_ok=True)
    actual_frame = corrupt_data[0, :, 0, :].reshape(1, -1).cpu().numpy()
    generated_frames = [x[0, :, 0, :].reshape(1, -1).cpu().numpy() for x in generated_xs]

    all_frames = np.concatenate([actual_frame] + generated_frames, axis=0)

    tsne = TSNE(n_components=3, perplexity=30, random_state=42)
    reduced_data = tsne.fit_transform(all_frames)

    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')

    ax.scatter(reduced_data[0, 0], reduced_data[0, 1], reduced_data[0, 2], c='#d77470', marker='o', label='Actual Frame')
    ax.scatter(reduced_data[1:, 0], reduced_data[1:, 1], reduced_data[1:, 2], c='#add2ca', marker='o', label='Diffusion Results')

    ax.set_title('Visualization of Frame and Diffusion Results')
    ax.legend()
    plt.savefig(os.path.join(output_dir, 'visualization_frame.png'), dpi=300, bbox_inches='tight')
    plt.show()
